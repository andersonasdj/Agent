...
public class ColetorSistema {
    ...
    public static DadosComputador coletarDados() {
     ...
     // Espaço em disco + Chaves de recuperação BitLocker
        ...
        for (OSFileStore fs : fileStores) {
            ...
            String mount = fs.getMount();         // Exemplo: "C:\\"
        ...
        dados.username = (sessions != null && !sessions.isEmpty()) ? sessions.get(0).getUserName()
        ...
        // Antivírus
        try {
            Process process = Runtime.getRuntime().exec("powershell -Command \"Get-CimInstance -Namespace 'root/SecurityCenter2' -ClassName AntivirusProduct | Select-Object displayName\"");
            BufferedReader reader = new BufferedReader(new InputStreamReader(process.getInputStream()));
